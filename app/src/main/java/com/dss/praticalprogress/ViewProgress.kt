...
class ViewProgress{
    ...
    fun load(){
        progressLayout = if (progressLayout == 0) R.layout.dialog_progress_local_white else progressLayout
        runProgressOnView()
    }


    fun runProgressOnView() {
        val inflater = LayoutInflater.from(activity)

        if (targetView is ViewGroup && targetView !is RecyclerView && targetView !is ScrollView) {
            inflater.inflate(R.layout.dialog_progress_local_white, targetView as ViewGroup, true)
